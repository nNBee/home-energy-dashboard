  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:gap-6 md:block">
        <p className="shrink-0 text-base font-semibold tracking-tight text-slate-900 dark:text-slate-100">
          Home Energy
        </p>
          ...
          {navigationItems.map(({ label, to }) => (
            <li key={to}>
              <NavLink
                ...
                className={({ isActive }) =>
                  [
                    'block rounded-md px-3 py-2 text-sm font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-600 dark:focus-visible:outline-emerald-400',
                    isActive
